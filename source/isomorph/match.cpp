...
#define MAXIMUM_COUNT_MATCH       32784 // 32K
static unsigned int COUNT_MATCH = 0;
 ...
 * in the two graphs
 ------------------------------------------------------------*/
bool match(State *s0, int *pn, node_id c1[], node_id c2[], int id)
  {
#ifdef __DEBUG__
		printf ("\nMatching for Thread ID: %d\n", id);
#endif
...
static bool match(int *pn, node_id c1[], node_id c2[], State *s)
  { if (s->IsGoal())
    ...
    bool found=false;
    while (COUNT_MATCH <= MAXIMUM_COUNT_MATCH && !found && s->NextPair(&n1, &n2, n1, n2))
      {
    	COUNT_MATCH++;
