...
#
# Prepares data (files) for n-fold cross validation using SWOD.
# Takes a number, to be used as n, two files benign and
# malware to generate files for testing DroidNative, a tool for
# malware detection, and exe of the tool DroidNative.
#
...
# AND
# full path name of the tool DroidNative
...
#
# Usage:
# run-cross-validation.pl <n> <file_name_benign_samples> <file_name_malware_samples> <file_name_DroidNative>
# Example:
# run-cross-validation.pl 10 benign_samples.txt malware_samples.txt DroidNative.exe
#
...
my $DroidNative_full_path = $ARGV[3];
...
if ($FINDING_BEST_VALUES == 0)
{
	...
	for (my $hsd_count = $STARTING; $hsd_count <= $COUNT; $hsd_count += $jump)
	{
		...
		for (my $i = 0; $i < $N; $i++)
		{
			my $cmd = sprintf("%s %d %.02f %.02f 1.00 1.00 %.02f %.02f malware_samples_%02d.txt benign_samples_%02d.txt virus_samples_%02d.txt files_to_check_%02d.txt > results_%03d_%02d.txt", $DroidNative_full_path, $max_threads, $VWOD, $HWOD, $VSD, $HSD, $i, $i, $i, $i, $hsd_count, $i);
			print "\nRunning $cmd\n";
			system ($cmd);
		}
	}
...
elsif ($FINDING_BEST_VALUES == 1)
{
	...
	for (my $i = 0; $i < $N; $i++)
	{
		my $cmd = sprintf("%s %d %.02f %.02f 1.00 1.00 %.02f %.02f malware_samples_%02d.txt benign_samples_%02d.txt virus_samples_%02d.txt files_to_check_%02d.txt > results_%02d.txt", $DroidNative_full_path, $max_threads, $VWOD, $HWOD, $VSD, $HSD, $i, $i, $i, $i, $i);
		print "\nRunning $cmd\n";
		system ($cmd);
	}
}
else
{
	...
	for (my $vsd_count = $STARTING_VSD; $vsd_count <= $V_COUNT; $vsd_count += $jump_vsd)
	{
		for (my $hsd_count = $STARTING_HSD; $hsd_count <= $H_COUNT; $hsd_count += $jump_hsd)
		{
			...
			for (my $i = 0; $i < $N; $i++)
			{
				my $cmd = sprintf("%s %d %.02f %.02f 1.00 1.00 %.02f %.02f malware_samples_%02d.txt benign_samples_%02d.txt virus_samples_%02d.txt files_to_check_%02d.txt > ./VSD_HSD/results_%02d_%03d_%02d.txt", $DroidNative_full_path, $max_threads, $VWOD, $HWOD, $VSD, $HSD, $i, $i, $i, $i, $vsd_count, $hsd_count, $i);
				print "\nRunning $cmd\n";
				system ($cmd);
			}
		}
	...
	my $max_count = 0;
	my $file_max_count = "";
	my $count_zeros_percentage = 0;
	my $max_count_zeros_percentage = 0;
	for (my $vsd_count = $STARTING_VSD; $vsd_count <= $V_COUNT; $vsd_count += $jump_vsd)
	{
		for (my $hsd_count = $STARTING_HSD; $hsd_count <= $H_COUNT; $hsd_count += $jump_hsd)
		{
			...
			$count_zeros_percentage = ($max_count_zeros / $MALWARE_sub_samples) * 100;
			if ($count_zeros_percentage >= $max_count_zeros_percentage and ($max_count_zeros + $max_count_ones) > $max_count)
			{
				$max_count_zeros_percentage = $count_zeros_percentage;
				$max_count = $max_count_zeros + $max_count_ones;
				$file_max_count = "$file";
				$global_count++;
			}
print "$count_zeros_percentage ? $max_count_zeros_percentage     MAX_COUNT: $max_count     FILE: $file_max_count\n";
		}
	}
...
#	my $STARTING_HWOD = $HWOD;
	...
	for (my $vwod_count = $STARTING_VWOD; $vwod_count <= $V_COUNT; $vwod_count += $jump_vwod)
	{
		for (my $hwod_count = $STARTING_HWOD; $hwod_count <= $H_COUNT; $hwod_count += $jump_hwod)
		{
			...
			for (my $i = 0; $i < $N; $i++)
			{
				my $cmd = sprintf("%s %d %.02f %.02f 1.00 1.00 %.02f %.02f malware_samples_%02d.txt benign_samples_%02d.txt virus_samples_%02d.txt files_to_check_%02d.txt > ./VWOD_HWOD/results_%02d_%03d_%02d.txt", $DroidNative_full_path, $max_threads, $VWOD, $HWOD, $VSD, $HSD, $i, $i, $i, $i, $vwod_count, $hwod_count, $i);
				print "\nRunning $cmd\n";
				system ($cmd);
			}
		}
	...
	$max_count = 0;
	$file_max_count = "";
	$count_zeros_percentage = 0;
	$max_count_zeros_percentage = 0;
	for (my $vwod_count = $STARTING_VWOD; $vwod_count <= $V_COUNT; $vwod_count += $jump_vwod)
	{
		for (my $hwod_count = $STARTING_HWOD; $hwod_count <= $H_COUNT; $hwod_count += $jump_hwod)
		{
			...
			my $max_count_ones = $count_ones;
			$count_zeros_percentage = ($max_count_zeros / $MALWARE_sub_samples) * 100;
			if ($count_zeros_percentage >= $max_count_zeros_percentage && ($max_count_zeros + $max_count_ones) > $max_count)
			{
				$max_count_zeros_percentage = $count_zeros_percentage;
				$max_count = $max_count_zeros + $max_count_ones;
				$file_max_count = "$file";
